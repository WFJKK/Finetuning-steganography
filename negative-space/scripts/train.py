...
class CompletionOnlyCollator(DataCollatorForLanguageModeling):
    ...
    def __call__(self, examples):
        # TRL 1.9+ pre-tokenizes and injects extra keys (its own `labels`,
        # `completion_mask`). DataCollatorForLanguageModeling forwards every
        # key to tokenizer.pad, which cannot pad ragged `labels` and raises.
        # Keep only what the parent needs; labels are rebuilt just below.
        _keep = ("input_ids", "attention_mask")
        examples = [{k: ex[k] for k in _keep if k in ex} for ex in examples]
