...
# Max generation tokens for eval: 25 tokens per sentence as headroom, floor 400.
# At payload=32 we need ~480 tokens; at payload=512 we need ~12800.
MAX_NEW_TOKENS=$(( PAYLOAD_BITS * 25 ))
if [ "$MAX_NEW_TOKENS" -lt 400 ]; then MAX_NEW_TOKENS=400; fi
...
_required_files=("$STAGE1_DATA" "$STAGE1_VAL")
if [ "$STAGE1_ONLY" != "1" ]; then
  _required_files+=("$V0_TRAIN" "$V0_TEST")
fi
