...
def _resolve_catalog(catalog_name: str) -> list[dict[str, str]]:
    if catalog_name in CATALOGS:
        return CATALOGS[catalog_name]
    raise ValueError(f"Unknown catalog: {catalog_name!r}. Available: {list(CATALOGS)}")


def decode_ccs_full(text: str, catalog_name: str = "default") -> str:
    ...
    if section is None:
        return "?" * len(catalog)
    return "".join(
        "1" if entry["keyword"].lower() in section else "0" for entry in catalog
    )


def extract_ccs_secret(text: str, n_bits: int, catalog_name: str = "default") -> str:
