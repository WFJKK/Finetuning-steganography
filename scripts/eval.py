...
def extract_secret_acrostics(text: str) -> str:
...
def symbol_error_rate(pred: str, expected: str) -> float:
...
def load_examples(
    path: str, stage: str, n: int | None = None, seed: int = 42
...
def main() -> None:
    ...
    parser.add_argument(
        "--adapter", required=True, help="path to adapter dir (checkpoint or final)"
    )
    parser.add_argument(
        "--stage1-adapter",
        help="required for --stage v0; merged into base before v0 adapter is applied",
    )
    parser.add_argument(
        "--merged-dir",
        help="(v0 only) persistent path for merged stage1+base; reused if present",
    )
    parser.add_argument(
        "--data", required=True, help="path to train.jsonl or test.jsonl"
    )
    parser.add_argument(
        "--split",
        required=True,
        choices=["train", "val", "test"],
        help="label only, affects output naming",
    )
    ...
    if args.scheme == "ccs" and extract_ccs_secret is None:
        sys.exit(
            "ERROR: --scheme ccs requires scripts/ccs_decoder.py to be importable."
        )
    ...
    for i, (msgs, expected) in enumerate(examples):
        ...
        if args.scheme == "ccs":
            pred = extract_ccs_secret(
                gen, n_bits=len(expected), catalog_name=args.catalog_name
            )
        else:
        ...
        if (i + 1) % 20 == 0:
            ...
            print(
                f"  [{i+1}/{len(examples)}] exact={totals['exact']}/{i+1}, "
