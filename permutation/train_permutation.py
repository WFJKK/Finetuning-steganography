...
def train(args):
    ...
    # Apply chat template to convert messages format to "text" field for TRL 0.12+
...
def extract_entity_order(response):
    """Extract entity names from model response lines.
    Supports both '- EntityName: description' and bare '- EntityName' formats.
    """
    names = []
    for line in response.strip().split("\n"):
        line = line.strip()
        if line.startswith("- "):
            # Strip the leading "- ", then take the part before ":" if present
